...
function bevestigInschrijving()
{
    ...
    else
    {
        ...
        const form1 = document.getElementById('idFormInschrijven');

        const inputVoornaam = document.getElementById('idVoornaamMail');
        const inputNaam = document.getElementById('idNaamMail');
        const inputPostcode = document.getElementById('idPostcodeMail');
        const inputGemeente = document.getElementById('idGemeenteMail');
        const inputGeslacht = document.getElementById('idGeslachtMail');
        const inputGeboortedatum = document.getElementById('idGeboortedatumMail');
        const inputEmail = document.getElementById('idEmailMail');
        const inputLoopclub = document.getElementById('idLoopclubMail');
        const inputBedrijf = document.getElementById('idBedrijfMail');
        const inputBokslid = document.getElementById('idBokslidMail');
        const inputStofwisselingsziekte = document.getElementById('idStofwisselingsziekteMail');
        const inputCategorie = document.getElementById('idCategorieMail');

        //Values correct zetten
        inputVoornaam.value = voornaam;
        inputNaam.value = naam;
        inputPostcode.value = postcodeZonderSpaties;
        inputGemeente.value = gemeente;
        inputGeslacht.value = geslacht;
        inputGeboortedatum.value = geboortedatumDatabase;
        inputEmail.value = email;
        inputLoopclub.value = loopclub;
        inputBedrijf.value = bedrijf;
        inputBokslid.value = bokslid;
        inputStofwisselingsziekte.value = stofwisselingsziekte;
        inputCategorie.value = categorie;

        const serviceID = 'default_service';
        const templateID = 'template_8nsw3xb';

        emailjs.sendForm(serviceID, templateID, form1)
            .then(() => {
                //Melding geven dat inschrijving geslaagd is.
                //New instance of showAlertBox
                alertBox = new showAlertBox();
                //alertbox weergeven
                alertBox.render("Inschrijving geregistreerd","Uw inschrijving is verwerkt, U ontvangt één van de komende dagen een bevestigingsmail.");
            }, (err) => {
